...
| 功能 | Chat Completion / Embeddings / Image Generation / ASR 语音识别 / MCP 工具调用 |
...
adapters:
  - type: ai
    ...
    config:
      ...
      # MCP 服务器（可选，让模型调用外部工具）
      mcp_servers:
        weather:
          transport: "http"                # http | sse | stdio（缺省自动判断）
...
| `mcp_servers` | dict | `{}` | MCP 服务器配置（详见下文"MCP 工具调用"） |
...
[MCP (Model Context Protocol)](https://modelcontextprotocol.io/) 让模型在对话中调用外部工具（查天气、搜网页、操作 GitHub 等）。配置 `mcp_servers` 后，`chat()` 会自动加载工具并把工具执行结果回传给模型，形成"请求工具 → 执行 → 回传 → 继续对话"的循环，直到模型给出最终回答。

::: tip 依赖
需安装 `mcp` Python SDK。AI 适配器启动时自动检查安装，或手动 `pip install "mcp>=1.25.0,<2.0.0"`。
:::

### 配置格式

`mcp_servers` 是 `{服务器名: 配置}` 字典，格式与 [LiteLLM MCP](https://docs.litellm.ai/docs/mcp) 兼容：

| 传输 | 适用 | 必需 | 可选 |
|------|------|------|------|
| `http` | Streamable HTTP（现代标准，默认） | `url` | `headers` |
| `sse` | 旧版 Server-Sent Events | `url` | `headers` |
| `stdio` | 本地子进程（npx / uvx / python） | `command` | `args`、`env` |
...
`transport` 缺省自动判断：有 `command` → `stdio`；有 `url` → `http`。多个服务器的工具按 `{服务器名}_{工具名}` 前缀区分，避免同名冲突（如 GitHub 的 `create_issue` → `github_create_issue`）。

### 使用方式

配置后 `chat()` 无需额外参数即可使用工具（默认读取 config）：

```python
resp = await self.api.ai.chat("给我看看 deepwiki 上关于 MCP 的首页内容")
print(resp.choices[0].message.content)
```

也可临时覆盖服务器配置，或用 `max_tool_calls`（默认 10）限制单轮工具调用轮数：

```python
resp = await self.api.ai.chat(
    "查询北京的天气",
    mcp_servers={"weather": {"transport": "http", "url": "https://mcp.weather.com/mcp"}},
    max_tool_calls=5,
)
```

模型不调用工具通常是模型不支持 function calling 或未配置 `mcp_servers`；工具报错时检查 `url`/`command` 与 `headers`/`env`。单个服务器连接失败不影响其他服务器，可查看日志中 `MCP 服务器 xxx 连接失败` 信息。
...
- `examples/ai/02_mcp_tools/` — MCP 工具调用（模型自动调用外部工具）
