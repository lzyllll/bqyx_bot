...
class ReplyService:
    ...
    async def send_help(self, event: GroupMessageEvent) -> None:
        try:
            ...
            await self._send_image(event, png_bytes)
            return
        except Exception:
            pass
        ...
        fc.attach_text(
            ...
            "  例：查成员 / 查成员 表格\n"
            "查日贡 [阈值] [图片|表格|文本] 查看今日达标/未达标成员\n"
            "  例：查日贡 / 查日贡 1100 / 查日贡@ 1100\n"
            "查周贡 [阈值] [图片|表格|文本] 查看本周达标/未达标成员\n"
            "  例：查周贡 / 查周贡 4000\n"
        ...
        *,
        ...
        if format_type == "图片":
            await self._send_image(
                event,
                self.domain_renderer.image(member_list, union_info, uid=uid),
            )
            return
        if format_type == "表格":
            await self._send_excel(
            ...
            return
        await event.reply(self.domain_renderer.text(member_list, union_info, uid=uid))
    ...
    async def send_union_info(
        ...
        if format_type == "图片":
            await self._send_image(event, self.union_info_renderer.image(union_info))
            return
        await event.reply(self.union_info_renderer.text(union_info))
